...
void AShooterProjectile::ExplosionCheck(const FVector& ExplosionCenter)
{
	...
	for (const FOverlapResult& CurrentOverlap : Overlaps)
	{
		...
		if (DamagedActors.Find(CurrentOverlap.GetActor()) == INDEX_NONE)
		{
			...
			FHitResult DummyHit;
			ProcessHit(CurrentOverlap.GetActor(), CurrentOverlap.GetComponent(), GetActorLocation(), ExplosionDir.GetSafeNormal(), DummyHit);
		}
	}
}
...
void AShooterProjectile::ProcessHit(AActor* HitActor, UPrimitiveComponent* HitComp, const FVector& HitLocation, const FVector& HitDirection, const FHitResult& Hit)
{
	...
	if (Enemy)
	{
		...
		float DistanceMeters = FVector::Distance(SpawnLocation, GetActorLocation()) / 100.0f;
		if (DistanceMeters > 1.0f) {
			Damage -= (DistanceMeters - 1.0f) * 10.0f;
		}
		...
		else if (BoneName.ToString().Contains("neck")) {
			Multiplier = 1.5f;
		}
		else if (BoneName.ToString().Contains("hand")) {
			Multiplier = 0.5f;
		}
		
		UGameplayStatics::ApplyDamage(Enemy, 
			FMath::Clamp(Damage, 0.0f, 50.0f) * Multiplier, 
